...
function Retry(Base) {
    ...
    return class Retry extends Base {
        ...
        match(rules, { Error }, fallback) {
            const matcher = ({ ErrorEquals }, _, rules) => {
                 ...
                 */
                if (rules.length === 1 && ErrorEquals.length === 1 && ErrorEquals[0] === 'States.ALL') {
                    return true;
                }
